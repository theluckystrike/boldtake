...
const SECURITY_CONFIG = {
  ...
  // Timing constraints (milliseconds) - NATURAL BEHAVIOR
  MIN_DELAY_BETWEEN_ACTIONS: 90000,   // 1.5 minutes minimum
  MAX_DELAY_BETWEEN_ACTIONS: 240000,  // 4 minutes maximum  
  BURST_COOLDOWN_DURATION: 300000,    // 5 minute cooldown if needed
...
async function processNextTweet() {
  ...
  if (success) {
    ...
    // CRITICAL: Like the tweet AFTER successful reply to prevent double commenting
    addDetailedActivity(`❤️ Liking the tweet to mark as processed...`, 'info');
    await sleep(2000); // Wait for modal to fully close and page to stabilize
    
    // Method 1: Try to find and click the like button on our last reply
    debugLog('🔍 Looking for our reply to like the original tweet...');
    
    // Find the most recent reply (should be ours)
    const allTweets = document.querySelectorAll('[data-testid="tweet"]');
    let ourReply = null;
    
    // Look for a tweet that contains our reply text (the one we just posted)
    if (replyText) {
      for (const t of allTweets) {
        const text = t.innerText || t.textContent || '';
        if (text.includes(replyText.substring(0, 30))) {
          ourReply = t;
          debugLog('✅ Found our reply on the page');
          break;
        }
      }
    } else {
      debugLog('⚠️ No reply text available to search for');
    }
    
    // If we found our reply, the parent tweet should be right above it
    let tweetToLike = null;
    if (ourReply) {
      // Navigate up to find the parent tweet (the one we replied to)
      let current = ourReply.previousElementSibling;
      while (current && !tweetToLike) {
        if (current.querySelector('[data-testid="tweet"]')) {
          tweetToLike = current.querySelector('[data-testid="tweet"]');
          debugLog('✅ Found parent tweet above our reply');
          break;
        }
    ...
    // Method 2: Re-find the original tweet by text content
    if (!tweetToLike) {
      const tweetText = tweet.innerText || tweet.textContent || '';
      const tweetSnippet = tweetText.substring(0, 50).trim();
      debugLog(`🔍 Re-finding tweet with text: "${tweetSnippet}..."`);
      
      // Try to find by author and partial text
      for (const t of allTweets) {
        const text = t.innerText || t.textContent || '';
        // More flexible matching - check if it contains part of the original text
        if (tweetSnippet && text.includes(tweetSnippet.substring(0, 30))) {
          tweetToLike = t;
          debugLog('✅ Found matching tweet by text content');
          
          // Debug: Check button status
          const likeBtn = t.querySelector('[data-testid="like"]');
          const unlikeBtn = t.querySelector('[data-testid="unlike"]');
          if (unlikeBtn) {
            debugLog('ℹ️ Tweet already liked');
          } else if (likeBtn) {
            debugLog('✅ Like button found on matched tweet');
            // Try to get the actual button element
            const actionBar = t.querySelector('[role="group"]');
            if (actionBar) {
              const buttons = actionBar.querySelectorAll('button');
              debugLog(`Found ${buttons.length} buttons in action bar`);
            }
          } else {
            debugLog('⚠️ No like/unlike button found');
          }
          break;
        }
      }
    ...
    // Fallback to original tweet element
    if (!tweetToLike) {
      debugLog('⚠️ Could not find tweet after reply, using original element');
      tweetToLike = tweet;
      
      // Check if the original element still has the button
      const likeBtn = tweet.querySelector('[data-testid="like"]');
      if (!likeBtn) {
        debugLog('❌ Original tweet element no longer has like button - DOM changed');
      }
    }
    
    // Try to like, but don't let it break the session
    try {
      const liked = await likeTweetSafely(tweetToLike); // Enhanced like function
      if (liked) {
        debugLog('✅ Tweet liked successfully - will not be processed again');
        addDetailedActivity(`✅ Tweet liked - marked as processed`, 'success');
      } else {
        debugLog('⚠️ Could not like tweet - marking as processed anyway to continue');
        addDetailedActivity(`⚠️ Like failed - continuing session`, 'warning');
        // Mark as processed anyway so we don't get stuck
        tweet.setAttribute('data-boldtake-processed', 'true');
        tweet.setAttribute('data-boldtake-liked-failed', 'true');
        if (tweetToLike && tweetToLike !== tweet) {
          tweetToLike.setAttribute('data-boldtake-processed', 'true');
          tweetToLike.setAttribute('data-boldtake-liked-failed', 'true');
        }
      }
    } catch (error) {
      errorLog('❌ Error during like attempt:', error);
      // Mark as processed anyway to continue
      tweet.setAttribute('data-boldtake-processed', 'true');
      tweet.setAttribute('data-boldtake-like-error', 'true');
    }
  } else {
    sessionStats.failed++;
