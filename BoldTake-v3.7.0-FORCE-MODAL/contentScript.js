...
const SECURITY_CONFIG = {
  ...
  // Timing constraints (milliseconds) - SMART SPEED MODE 🎯
  MIN_DELAY_BETWEEN_ACTIONS: 60000,   // 1 minute minimum - Fast but safe
  MAX_DELAY_BETWEEN_ACTIONS: 180000,  // 3 minutes maximum - Human-like
  BURST_COOLDOWN_DURATION: 180000,    // 3 minute cooldown - Protective
  
  // Advanced behavioral patterns - SMART & NATURAL
  HUMAN_VARIANCE_FACTOR: 0.4, // 40% variance - More natural
  BREAK_PROBABILITY: 0.10,     // 10% chance - Reasonable breaks
  LONG_BREAK_DURATION: 300000, // 5 minute break - Normal human pause
  MICRO_BREAK_PROBABILITY: 0.15, // 15% - Natural micro-pauses
  MICRO_BREAK_DURATION: 90000,   // 1.5 minute micro-breaks - Realistic
...
function calculateHumanProcessingDelay() {
  const delayConfig = {
    minDelay: 60000,     // 1 minute minimum - Smart speed
    maxDelay: 180000,    // 3 minutes maximum - Natural pace
    baseDelay: 120000,   // 2 minutes average - Balanced
    variationFactor: 0.4 // ±40% variation - Human-like
  };
  ...
  debugLog('🎯 SMART SPEED DELAY', {
    delaySeconds: Math.round(finalDelay / 1000),
    range: '60-180 seconds (1-3 minutes)',
    behavior: 'Fast but SAFE',
...
async function startContinuousSession(isResuming = false) {
  ...
  // URL PROTECTION: Save starting URL and validate we're on a search page
  const startingUrl = window.location.href;
  debugLog('📍 Starting URL:', startingUrl);
  
  // Prevent starting on /explore page without search query
  if (startingUrl.includes('/explore') && !startingUrl.includes('?q=')) {
    errorLog('❌ Cannot start session on /explore page!');
    showStatus('❌ Navigate to a search page with tweets first!');
    sessionStats.isRunning = false;
    return;
  }
  ...
  if (!isResuming) {
    ...
    sessionStats = {
      ...
      totalDelay: 0,              // STEALTH: Track total delay for pattern detection
      recentActions: [],          // STEALTH: Track recent action timestamps
      hourlyActions: []           // STEALTH: Track hourly action timestamps
    };
  ...
  try {
    while (sessionStats.isRunning && sessionStats.processed < sessionStats.target) {
      ...
      // URL PROTECTION: Stop if we've navigated away from search
      const currentUrl = window.location.href;
      if (currentUrl.includes('/explore') && !currentUrl.includes('?q=')) {
        errorLog('❌ Navigated away from search page - stopping session!');
        showStatus('❌ Lost search page - please navigate back to search!');
        sessionStats.isRunning = false;
        break;
      }
...
function assessAccountRisk() {
  ...
  // STEALTH MODE: Much more intelligent risk assessment
  
  // Burst activity check (REALISTIC THRESHOLDS)
  // Normal users can do 5-10 actions in 10 minutes
  if (recentActions >= 10) riskScore += 10;  // Light warning
  if (recentActions >= 15) riskScore += 20;  // Moderate concern
  if (recentActions >= 20) riskScore += 40;  // High concern
  
  // Hourly activity check (HUMAN-LIKE LIMITS)
  // Normal users might do 20-30 actions per hour
  if (hourlyActions >= 25) riskScore += 10;   // Starting to be active
  if (hourlyActions >= 35) riskScore += 20;   // Very active
  if (hourlyActions >= 45) riskScore += 40;   // Suspiciously active
  
  // Failure rate check (FORGIVING)
  // Some failures are normal on X.com
  if (failureRate > 0.6) riskScore += 10;   // Many failures
  if (failureRate > 0.8) riskScore += 20;   // Most failing
  
  // Consecutive actions check (REMOVED - too sensitive)
  // if (sessionStats.retryAttempts >= 2) riskScore += 30;
  
  // Time-based risk (NEW - smarter detection)
  ...
  // If we're doing exact same timing repeatedly, that's bot-like
  if (sessionStats.processed > 5) {
    const avgDelay = sessionStats.totalDelay / sessionStats.processed;
    if (avgDelay < 45000) { // Less than 45 seconds average
      riskScore += 20; // Too fast
    }
  }
  
  // Determine risk level (MUCH HIGHER THRESHOLDS)
  if (riskScore >= 120) { // Was 80
    riskLevel = 'critical';
    sessionStats.isRunning = false; // EMERGENCY STOP
    errorLog('🚨 Critical risk detected - pausing for safety');
    addDetailedActivity('⚠️ Safety pause activated', 'warning');
  } else if (riskScore >= 90) { // Was 60
    riskLevel = 'high';
  } else if (riskScore >= 60) { // Was 40
    riskLevel = 'medium';
  }
...
async function processNextTweet() {
  ...
  if (riskLevel === 'high') {
    ...
    await sleep(30000); // 30 second safety delay - TURBO MODE!
  }
  ...
  debugLog('👍 Liking tweet BEFORE opening reply modal...');
  ...
  // FORCE MODAL: Prevent X.com from opening popup by simulating trusted click
  // Re-calculate rect for accuracy (element might have moved)
  const modalRect = replyButton.getBoundingClientRect();
  const clickEvent = new MouseEvent('click', {
    view: window,
    bubbles: true,
    cancelable: false,
    clientX: modalRect.left + modalRect.width/2,
    clientY: modalRect.top + modalRect.height/2,
    screenX: modalRect.left + modalRect.width/2,
    screenY: modalRect.top + modalRect.height/2,
    button: 0,
    buttons: 1,
    ctrlKey: false,
    altKey: false,
  ...
  // Try to force modal instead of popup
  replyButton.focus();
  await sleep(100);
  ...
  // STEALTH: Track the delay for pattern detection
  sessionStats.totalDelay = (sessionStats.totalDelay || 0) + humanDelay;
  
  const delaySeconds = Math.round(humanDelay / 1000);
  const delayMinutes = Math.round(delaySeconds / 60 * 10) / 10; // Round to 1 decimal
  debugLog(`🥷 STEALTH DELAY: ${delayMinutes}m (${delaySeconds}s) - Undetectable!`);
  addDetailedActivity(`⏱️ Stealth delay ${delayMinutes}m (anti-detection)`, 'info');
  
  // Update status to show STEALTH timing
  showStatus(`🥷 Next tweet in ${delayMinutes}m (Stealth Mode)`);
...
async function handleReplyModal(originalTweet) {
  ...
  if (isNewWindow) {
    debugLog('🪟 Popup detected - attempting to type anyway!');
    addDetailedActivity('🔄 Working in popup window', 'info');
    
    // NEW STRATEGY: Try to work IN the popup window
    // Give it more time to load since it's a new window
    await sleep(3000);
    
    // Don't close immediately - try to find the text area
    // The popup should have the text area ready
  }
...
criticalLog('⚡ BoldTake v3.7.0 FORCE-MODAL - Works Even in Popups!');
criticalLog('🎯 Forces modal opening + works IN popups if needed');
criticalLog('💪 Dual strategy: Prevent popups OR work inside them');
criticalLog('🛡️ Smart risk detection (won\'t panic after 3 tweets)');
criticalLog('📍 Handles X.com\'s aggressive popup behavior');
criticalLog('✅ Will complete replies even if popup opens!');
