...
async function processNextTweet() {
  ...
  if (success) {
    ...
    // Re-find the tweet element after modal close as DOM might have changed
    ...
    // Find all tweets on page and match by text content
    ...
    for (const t of allTweets) {
      ...
      if (tweetSnippet && text.includes(tweetSnippet)) {
        currentTweet = t;
        debugLog('✅ Found matching tweet after modal close');
        break;
      }
    }
    
    // Use the re-found tweet or fallback to original
    const tweetToLike = currentTweet || tweet;
    ...
    if (!likeButton) {
      ...
      for (const btn of buttons) {
        ...
        if (ariaLabel && ariaLabel.toLowerCase().includes('like')) {
          likeButton = btn;
          debugLog('🔍 Found like button by aria-label');
          break;
        }
      }
    }
