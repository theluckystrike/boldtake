...
const ERROR_BOUNDARY = {
    ...
    const now = Date.now();
        ...
        if (typeof addDetailedActivity === 'function') {
            addDetailedActivity(`⚠️ Error in ${context}: ${error.message}`, 'error');
        }
    ...
    triggerEmergencyStop(reason) {
        console.error(`🚨 BoldTake Emergency Stop: ${reason}`);
        ...
        if (typeof showStatus === 'function') {
            showStatus(`🚨 Session stopped: ${reason}`);
        }
        if (typeof addDetailedActivity === 'function') {
            addDetailedActivity(`🚨 Emergency stop: ${reason}`, 'error');
        }
    },
...
// 💾 BULLETPROOF SESSION PERSISTENCE SYSTEM
const SESSION_PERSISTENCE = {
    ...
    saveSessionState() {
        try {
            ...
            debugLog('💾 Session state saved:', sessionData);
    ...
    async recoverSession() {
        try {
            ...
            if (!savedSession) {
                debugLog('💾 No saved session to recover');
                return false;
            }
            ...
            if (shouldRecover) {
                console.log('🔄 RECOVERING SESSION: Restoring progress from crash/refresh');
                console.log(`📊 Progress: ${savedSession.successful}/${savedSession.target} replies completed`);
                ...
                if (typeof showStatus === 'function') {
                    showStatus(`🔄 Session recovered: ${savedSession.successful}/${savedSession.target} completed`);
                }
                if (typeof addDetailedActivity === 'function') {
                    addDetailedActivity(`🔄 Session recovered from crash/refresh - continuing from ${savedSession.successful}/${savedSession.target}`, 'success');
                }
                
                // Auto-restart session after 3 seconds
                setTimeout(() => {
                    if (typeof startContinuousSession === 'function') {
                        console.log('🚀 AUTO-RESTARTING recovered session...');
                ...
                debugLog('💾 Saved session too old or already completed, starting fresh');
                return false;
            }
    ...
    clearSavedSession() {
        ...
        debugLog('💾 Saved session cleared');
    },
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'BOLDTAKE_START') {
    ...
    // GAUNTLET TEST 3: Handle priority stop with immediate response
    const isHighPriority = message.priority === 'high';
    const requestId = message.requestId || 'unknown';
    
    debugLog(`🛑 ${isHighPriority ? 'HIGH PRIORITY ' : ''}Force stopping BoldTake session... [${requestId}]`);
    
    // INSTANT STOP: Set flag immediately to halt all loops
    sessionStats.isRunning = false;
    
    // GAUNTLET TEST 3: Clear ALL timers and intervals immediately
    const timersCleared = [];
    
    if (window.boldtakeCountdownInterval) {
      clearInterval(window.boldtakeCountdownInterval);
      window.boldtakeCountdownInterval = null;
      timersCleared.push('countdown');
    }
    
    if (window.boldtakeTimeout) {
      clearTimeout(window.boldtakeTimeout);
      window.boldtakeTimeout = null;
      timersCleared.push('timeout');
    }
    
    // Clear any other potential timers
    if (window.boldtakeDelayTimeout) {
      clearTimeout(window.boldtakeDelayTimeout);
      window.boldtakeDelayTimeout = null;
      timersCleared.push('delay');
    }
    
    // Force close any open modals (non-blocking)
    try {
      const closeButton = document.querySelector('[data-testid="app-bar-close"]');
      if (closeButton) {
        closeButton.click();
        timersCleared.push('modal');
      }
    } catch (e) {
    ...
    console.log(`✅ [${requestId}] Stop completed - cleared: ${timersCleared.join(', ')}`);
    
    sendResponse({
      success: true, 
      message: 'BoldTake session force stopped',
      timersCleared: timersCleared,
      priority: isHighPriority ? 'high' : 'normal',
      requestId
    });
  // REMOVED: GET_AUTH_TOKEN handler - no longer needed with standalone authentication
  ...
  try {
    while (sessionStats.isRunning && sessionStats.processed < sessionStats.target) {
      ...
      if (sessionStats.isRunning && sessionStats.processed < sessionStats.target) {
        ...
        debugLog(`⏰ Waiting ${minutes}m ${seconds}s before next tweet...`);
...
async function attemptGeneration(promptTemplate, tweetText) {
  try {
    // Get personalization settings for the API call
    const personalization = await getPersonalizationSettings();
    
    const response = await chrome.runtime.sendMessage({
      type: 'GENERATE_REPLY',
      tweetText: tweetText.slice(0, 1500), // Send raw tweet text
      persona: personalization.persona || 'professional',
      language: personalization.language || 'english', 
      tone: personalization.tone || 'professional',
      strategy: 'Engagement Indie Voice', // Will be enhanced with actual strategy
      prompt: promptTemplate.replace('{TWEET}', tweetText.slice(0, 1500)) // Keep for backward compatibility
    });
