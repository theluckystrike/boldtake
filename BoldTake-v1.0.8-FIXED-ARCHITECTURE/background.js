...
const config = BoldTakeConfig.config;
const { debugLog, errorLog, getApiUrl, getConfig } = BoldTakeConfig;

// Configuration shortcuts
const API_CONFIG = getConfig('api');
const SESSION_CONFIG = getConfig('session');
const STORAGE_CONFIG = getConfig('storage');
...
async function generateReplyWithSupabase(prompt, tweetContext = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      try {
        const response = await fetch(getApiUrl(API_CONFIG.generateReply), {
          method: 'POST',
          headers: {
