...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  try {
    ...
    if (message.type === 'DAILY_LIMIT_REACHED') {
      // Forward daily limit message to popup for UI update
      console.log('📊 Daily limit reached - forwarding to popup for UI update');
      chrome.runtime.sendMessage({
        type: 'SHOW_DAILY_LIMIT_UI',
        limit: message.limit || 125
      }).catch(() => {
        // Popup might not be open, that's okay
        console.log('Popup not open - daily limit message queued');
      });
