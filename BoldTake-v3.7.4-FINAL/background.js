...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  try {
    ...
    // CHECK AUTH STATUS - v3.7.4 FIX
    if (message.type === 'CHECK_AUTH_STATUS') {
      (async () => {
        try {
          const authData = await chrome.storage.local.get(['sb-ckeuqgiuetlwowjoecku-auth-token']);
          const hasAuth = authData && authData['sb-ckeuqgiuetlwowjoecku-auth-token'];
          
          if (hasAuth) {
            // Parse token to check expiry
            const token = JSON.parse(authData['sb-ckeuqgiuetlwowjoecku-auth-token']);
            const expiresAt = token.expires_at || 0;
            const now = Math.floor(Date.now() / 1000);
            const isExpired = expiresAt > 0 && expiresAt < now;
            
            debugLog(`Auth check: authenticated=${!isExpired}, expired=${isExpired}`);
