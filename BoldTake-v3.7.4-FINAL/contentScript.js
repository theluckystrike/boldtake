...
async function startContinuousSession(isResuming = false) {
  ...
  if (sessionStats.isRunning && !isResuming) {
    debugLog('⚠️ Session check: isRunning=true, checking if stuck...');
    
    // FIX v3.7.4: If session has been "running" but no progress in 5 minutes, reset it
    const lastActivity = sessionStats.lastActivityTime || 0;
    const timeSinceActivity = Date.now() - lastActivity;
    
    if (timeSinceActivity > 300000) { // 5 minutes
      debugLog('🔄 Previous session appears stuck, restarting...');
      sessionStats.isRunning = false;
      sessionStats.lastActivityTime = Date.now();
...
async function processNextTweet() {
  ...
  if (success) {
    ...
    sessionStats.lastActivityTime = Date.now(); // Track for stuck session detection
...
async function sendReplyWithKeyboard() {
  debugLog('🚀 Attempting to send reply...');
  
  // FIX: Try multiple methods to send the reply
  
  // Method 1: Find and click the Post/Reply button
  const postButtons = [
    '[data-testid="tweetButton"]',
    '[data-testid="tweetButtonInline"]', 
    'button[data-testid*="tweet"]',
    'div[role="button"][tabindex="0"] span:contains("Post")',
    'div[role="button"][tabindex="0"] span:contains("Reply")',
    '[aria-label*="Post"]',
    '[aria-label*="Reply"]'
  ];
    ...
    try {
      ...
      if (button && !button.disabled) {
        ...
        if (!document.querySelector('[data-testid="tweetTextarea_0"]')) {
          debugLog('✅ Reply sent successfully via button click');
          return true;
        }
      }
    } catch (e) {
  ...
  debugLog('⚠️ Standard selectors failed, searching by text...');
  const allButtons = document.querySelectorAll('div[role="button"], button');
  for (const btn of allButtons) {
    const text = btn.textContent.toLowerCase();
    if ((text === 'post' || text === 'reply') && !btn.disabled) {
      debugLog('✅ Found Post/Reply button by text content');
      btn.click();
      await sleep(500);
      
      if (!document.querySelector('[data-testid="tweetTextarea_0"]')) {
        debugLog('✅ Reply sent via text-based button click');
        return true;
      }
    }
  }
  ...
  debugLog('⚠️ Button clicks failed, trying keyboard shortcut...');
  ...
  try {
    ...
    errorLog('❌ All send methods failed:', error);
    return false;
  }
}
...
async function findTweet() {
  ...
  for (let tweet of tweets) {
    ...
    // CRITICAL: Check for blocked authors and reply restrictions
    const replyButton = tweet.querySelector('[data-testid="reply"]');
    
    // FIX v3.7.4: Check if author has blocked us
    if (tweet.textContent.includes('blocked you') ||
        tweet.textContent.includes('This author has blocked you') ||
        tweet.textContent.includes("can't perform this action")) {
      debugLog('🚫 Skipping tweet - author has blocked us');
      addDetailedActivity('🚫 Skipped - blocked by author', 'warning');
      tweet.setAttribute('data-boldtake-processed', 'true');
      tweet.setAttribute('data-boldtake-blocked', 'true');
      continue;
    }
    
    // FIX v3.7.4: Check for "Who can reply?" restrictions
    if (tweet.textContent.includes('People the author mentioned can reply') ||
        tweet.textContent.includes('Who can reply?') ||
        tweet.textContent.includes('Only people') ||
        (replyButton && replyButton.getAttribute('aria-label') && 
         replyButton.getAttribute('aria-label').includes('can reply'))) {
      debugLog('🚫 Skipping tweet with reply restrictions');
      addDetailedActivity('🚫 Skipped - reply restricted', 'warning');
      tweet.setAttribute('data-boldtake-processed', 'true');
      tweet.setAttribute('data-boldtake-restricted', 'true');
      continue;
    }
...
criticalLog('🔥 BoldTake v3.7.4 ULTIMATE - All Issues Fixed!');
criticalLog('✅ Fixed: Detects blocked authors & restricted replies');
criticalLog('✅ Fixed: Reply button clicking in popups');
criticalLog('✅ Fixed: Session management & auth checks');
criticalLog('📍 Smart: 20-40 tweets/hour natural pace');
criticalLog('💪 Most stable version yet!');
