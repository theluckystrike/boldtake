...
async function processNextTweet() {
  ...
  if (success) {
    ...
    for (const t of allTweets) {
      ...
      if (text.includes(reply.substring(0, 30))) {
        ourReply = t;
    ...
    if (!tweetToLike) {
      ...
      for (const t of allTweets) {
        const text = t.innerText || t.textContent || '';
        if (tweetSnippet && text.includes(tweetSnippet)) {
          tweetToLike = t;
          debugLog('✅ Found matching tweet by text content');
          
          // Debug: Check if like button exists
          ...
          if (likeBtn) {
            ...
            debugLog('ℹ️ Tweet already liked (unlike button found)');
          } else {
            debugLog('⚠️ No like/unlike button found on matched tweet');
          }
          break;
        }
      }
...
async function likeTweetSafely(tweet) {
  try {
    ...
    if (!likeButton) {
      errorLog('❌ Like button not found on tweet - trying all buttons');
      // Debug: log all button aria-labels
      const allButtons = tweet.querySelectorAll('button');
      allButtons.forEach(btn => {
        const label = btn.getAttribute('aria-label');
        if (label) debugLog(`Button aria-label: ${label}`);
      });
      return false;
    }
