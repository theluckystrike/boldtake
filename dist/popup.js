...
popupDebugLog('🚀 BoldTake Professional popup loaded');
    ...
    return new Promise((resolve, reject) => {
        const checkDependencies = () => {
            ...
            if (hasSupabase && hasBoldTakeAuth && hasBoldTakeAuthManager) {
                popupDebugLog('✅ All dependencies loaded successfully');
                resolve();
            } else {
                popupDebugLog(`⏳ Waiting for dependencies... Supabase: ${hasSupabase}, Auth: ${hasBoldTakeAuth}, AuthManager: ${hasBoldTakeAuthManager}`);
...
async function autoRefreshSubscriptionOnOpen() {
    try {
        ...
        if (!window.BoldTakeAuthManager) {
            popupDebugLog('🔍 Auth manager not available, skipping auto-refresh');
            return;
        }
        
        const authState = window.BoldTakeAuthManager.getAuthState();
        if (!authState.isAuthenticated) {
            popupDebugLog('🔍 User not authenticated, skipping auto-refresh');
            return;
        }
        
        popupDebugLog('🔄 AUTO-REFRESH: Checking subscription status on popup open...');
        popupDebugLog('💡 A+++ FEATURE: Auto-refreshing subscription for seamless post-payment experience');
        
        // Force fresh subscription check (ignores cache)
        await window.BoldTakeAuthManager.refreshSubscriptionStatus();
        popupDebugLog('✅ Auto-refresh completed successfully');
        
    } catch (error) {
        // Silent failure - don't disrupt user experience
        popupDebugLog('⚠️ Auto-refresh failed (non-critical):', error);
    }
}

// Initialize popup when DOM is loaded
document.addEventListener('DOMContentLoaded', async () => {
    console.log('📱 Initializing BoldTake Professional interface...');
    
    // CRITICAL: Check if required dependencies are loaded
    if (typeof window.BoldTakeAuth === 'undefined') {
        console.error('❌ BoldTakeAuth not loaded - checking scripts...');
        console.error('Available window properties:', Object.keys(window).filter(key => key.includes('BoldTake')));
        showInitializationError('Authentication system not loaded. Please reload extension.');
        return;
    }
    
    if (typeof window.BoldTakeAuthManager === 'undefined') {
        console.error('❌ BoldTakeAuthManager not loaded');
        showInitializationError('Authentication manager not loaded. Please reload extension.');
        return;
    }
    
    // PHASE 1: Initialize Authentication System with proper loading checks
    try {
        console.log('✅ Dependencies loaded, initializing authentication...');
    ...
    // Set up daily target change listener
    if (dailyTargetInput) {
        dailyTargetInput.addEventListener('change', updateDailyLimitDisplay);
    ...
    chrome.storage.local.get([
        ...
        if (result.boldtake_language) {
            ...
            popupDebugLog('🌍 Language preference loaded:', result.boldtake_language);
        }
    ...
    languageSelect.addEventListener('change', (e) => {
        popupDebugLog('🌍 Language changed to:', e.target.value);
        savePersonalizationSettings();
    });
    ...
    if (timingSelect) {
        timingSelect.addEventListener('change', (e) => {
            ...
            if (e.target.selectedOptions[0].disabled) {
                ...
                popupDebugLog('🚫 Premium feature: Advanced timing options restricted to current tier');
                return;
            }
            // Save timing preference if valid
            chrome.storage.local.set({ 'boldtake_timing_range': e.target.value });
            popupDebugLog('⏱️ Timing range updated:', e.target.value);
        });
    }
    ...
    if (dismissGuideBtn) {
        dismissGuideBtn.addEventListener('click', () => {
            ...
            popupDebugLog('🚫 Quick start guide dismissed permanently');
        });
    }
    ...
    if (quickSettingsBtn) {
        quickSettingsBtn.addEventListener('click', () => {
            ...
            // Save preference
            chrome.storage.local.set({ boldtake_hide_quick_guide: true });
            popupDebugLog('⚡ Quick setup opened, guide hidden');
        });
    }
    ...
    if (result.boldtake_hide_quick_guide && quickStartGuide) {
        quickStartGuide.classList.add('hidden');
        popupDebugLog('🚫 Quick start guide hidden per user preference');
    }
    ...
    if (shouldAutoOpenDashboard) {
        ...
        popupDebugLog('🎯 Auto-opened dashboard for session status visibility');
    }
    ...
    popupDebugLog('✅ BoldTake Professional popup ready!');
});
...
/**
 * Update the daily limit display in the dashboard
 */
function updateDailyLimitDisplay() {
    const dailyLimitDisplay = document.getElementById('dailyLimitDisplay');
    if (dailyTargetInput && dailyLimitDisplay) {
        const selectedValue = dailyTargetInput.value;
        dailyLimitDisplay.textContent = `/ ${selectedValue}`;
        popupDebugLog('📊 Updated daily limit display to:', selectedValue);
    }
}
...
async function startSession() {
    ...
    if (startBtn.disabled) {
        console.log('⚠️ Start session already in progress, ignoring duplicate click');
        return;
    }
    ...
    if (dashboardTab && dashboardPanel) {
        ...
        // Clear activity log and show patience message
        activityLog = [];
        addActivity('🚀 Session starting - be patient and watch the magic!', 'success');
        addActivity('⏱️ First reply in under 1 minute (optimized speed)', 'info');
        addActivity('🎯 Target: 20-30 replies per hour', 'info');
        addActivity('💡 The bot works at an optimized, natural pace', 'info');
    ...
    try {
        ...
        if (!keyword) {
            ...
            if (rotationKeywords && rotationKeywords.length > 0) {
                ...
                popupDebugLog('🎯 Using first rotation keyword:', keyword);
            } else {
                // Use popular default keyword
                keyword = 'startup';
                popupDebugLog('🎯 Using default keyword: startup');
            }
        }
        ...
        if (!minFaves || minFaves === '') {
            ...
            popupDebugLog('🎯 Using default engagement: 500+ likes');
        }
        ...
        popupDebugLog('🎯 Enhanced query:', enhancedQuery);

        // Find an existing X.com tab or create a new one
        try {
            const tabs = await chrome.tabs.query({ url: "*://x.com/*" });
            if (tabs.length > 0) {
                // Update existing X.com tab
                await chrome.tabs.update(tabs[0].id, { url: searchURL, active: true });
                popupDebugLog('✅ Updated existing X.com tab');
            } else {
                // Create new X.com tab
                await chrome.tabs.create({ url: searchURL, active: true });
                popupDebugLog('✅ Created new X.com tab');
            }
        } catch (error) {
            popupDebugLog('❌ Error opening X.com tab:', error);
            // Fallback: try to create tab anyway
            try {
                await chrome.tabs.create({ url: searchURL, active: true });
                popupDebugLog('✅ Fallback: Created new X.com tab');
            } catch (fallbackError) {
                popupDebugLog('💥 Complete failure to open X.com tab:', fallbackError);
                showAlert('Unable to open X.com. Please open X.com manually and try again.');
                resetStartButton();
                return;
            }
        }
        ...
        popupDebugLog('💥 Error starting session:', error);
...
async function stopSession() {
    popupDebugLog('🛑 Stopping BoldTake session...');
    ...
    try {
        ...
        chrome.tabs.sendMessage(tab.id, { type: 'BOLDTAKE_STOP' }, (response) => {
            if (chrome.runtime.lastError) {
                popupDebugLog('Content script not available, session likely already stopped');
            }
    ...
    try {
        ...
        chrome.tabs.sendMessage(tab.id, { type: 'GET_SESSION_STATS' }, (response) => {
            if (chrome.runtime.lastError) {
                ...
                popupDebugLog('❌ Error getting session stats:', chrome.runtime.lastError.message);
                return;
            }
            
            if (response && response.stats) {
                popupDebugLog('📊 Received session stats:', response.stats);
                updateSessionDisplay(response.stats);
            } else {
                popupDebugLog('⚠️ No stats received from content script');
            }
        });
        
    } catch (error) {
        // Silently handle errors
        popupDebugLog('📊 Content script not ready yet...');
    }
}
...
function updateSessionDisplay(stats) {
    popupDebugLog('🔄 Updating session display with stats:', stats);
    
    // Update counters
    const successful = stats.successful || 0;
    successfulCount.textContent = successful;
    popupDebugLog('📊 Updated successful count to:', successful);
...
function savePromptPreference(strategyName, variationId) {
    chrome.storage.local.get(['boldtake_prompt_preferences'], (result) => {
        ...
        chrome.storage.local.set({ boldtake_prompt_preferences: preferences }, () => {
            popupDebugLog(`💾 Saved prompt preference: ${strategyName} -> ${variationId}`);
        });
    });
}
...
function resetPromptPreferences() {
    if (confirm('Reset all prompt variations to defaults?')) {
        chrome.storage.local.remove('boldtake_prompt_preferences', () => {
            ...
            popupDebugLog('🔄 Reset all prompt preferences to defaults');
        });
    }
}
...
function savePersonalizationSettings() {
    ...
    popupDebugLog(`🌍 Settings updated Language=${language}, Tone=${tone}`);
}
...
function setupTabNavigation() {
    ...
    tabButtons.forEach(button => {
        ...
        settingsPanel.classList.remove('hidden');
            ...
            popupDebugLog(`📑 Switched to ${targetTab} tab`);
        });
    });
    ...
    if (dashboardTab) {
        dashboardTab.click();
        popupDebugLog('🎯 Dashboard tab shown by default');
    }
}
...
async function handleCSVUpload(event) {
    ...
    try {
        ...
        popupDebugLog('📊 CSV uploaded:', csvData.length, 'replies');
        ...
        if (tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) {
            chrome.tabs.sendMessage(tab.id, {
                ...
                if (response?.success) {
                    ...
                    popupDebugLog('❌ Analysis failed:', response?.error);
                }
            });
        } else {
        ...
        popupDebugLog('❌ CSV processing error:', error);
...
function displayAnalysisResults(insights) {
    ...
    popupDebugLog('🧠 Analysis results displayed:', insights);
}
...
async function applyAIImprovements() {
    try {
        ...
        if (tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) {
            chrome.tabs.sendMessage(tab.id, {
                ...
                if (response?.success) {
                    ...
                    popupDebugLog('🚀 AI improvements applied:', response.enhancements);
    } else {
        ...
        popupDebugLog('❌ Error applying insights:', error);
        ...
        async function handleLoginSubmission() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    popupDebugLog('✅ Login successful');
                } else {
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async () => {
            try {
                ...
                popupDebugLog('✅ Logout successful');
            } catch (error) {
    ...
    if (refreshSubscriptionBtn) {
        refreshSubscriptionBtn.addEventListener('click', async () => {
            try {
                ...
                popupDebugLog('🔄 Manual subscription refresh triggered');
                popupDebugLog('📧 Refreshing for current user');
                
                // FORCE fresh API call (no cache)
                await window.BoldTakeAuthManager.refreshSubscriptionStatus();
                popupDebugLog('✅ Subscription status forcefully refreshed');
...
function saveFilteringSettings() {
    ...
    popupDebugLog('🎯 Filtering settings saved:', { 
        excludeLinksMedia, 
        prioritizeQuestions, 
        minCharCount, 
...
async function loadAnalyticsData() {
    try {
        ...
        popupDebugLog('❌ Error loading analytics:', error);
    }
}
...
function updatePerformanceDashboard(data) {
    ...
    popupDebugLog('📊 Analytics dashboard is now a Bold Tier premium feature - showing demo data');
}
            ...
            try {
                ...
                if (tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) {
                    ...
                    chrome.tabs.sendMessage(tab.id, {type: 'SCRAPE_ANALYTICS'}, (response) => {
                        if (response?.success) {
                            popupDebugLog('📊 Analytics scraping triggered on startup');
                            sessionStorage.setItem('boldtake_analytics_scraped', 'true');
                        }
                    });
                }
            } catch (error) {
                popupDebugLog('Analytics scraping not available:', error.message);
            }
...
function clearCommentHistory() {
    if (confirm('Are you sure you want to clear all comment history? This cannot be undone.')) {
        ...
        loadAnalyticsData();
        popupDebugLog('🗑️ Comment history cleared');
    }
}
...
function handleNicheSelection() {
    ...
    suggestedKeywords.querySelectorAll('.suggestion-keyword-chip').forEach(chip => {
        chip.addEventListener('click', () => {
            ...
            popupDebugLog(`📝 Keyword suggestion selected: ${keyword}`);
        });
    });
}
...
popupDebugLog('✅ BoldTake Professional popup script ready!');
