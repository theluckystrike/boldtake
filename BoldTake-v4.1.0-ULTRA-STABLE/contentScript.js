...
async function processNextTweet() {
  ...
  while (attempt < maxAttempts) {
    ...
    if (tweet) {
      ...
      resetWatchdog(); // Reset watchdog when we find a tweet
  ...
  if (success) {
    ...
    // STABILITY: Reset watchdog on successful progress
    resetWatchdog();
...
// STABILITY: Add watchdog timer to prevent getting stuck
...
function resetWatchdog() {
  ...
  // Set watchdog to trigger if no progress for 5 minutes
  watchdogTimer = setTimeout(() => {
    if (sessionStats.isRunning) {
      debugLog('🚨 WATCHDOG: Extension appears stuck, attempting recovery...');
      sessionLog('🔄 Watchdog recovery triggered', 'warning');
