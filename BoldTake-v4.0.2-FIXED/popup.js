...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Update daily limit display when tier changes
    if (dailyTargetInput) {
...
async function startSession() {
    ...
    // Show encouraging messages in the activity log
    activityLog = [];
...
/**
 * Pauses the current BoldTake session
 */
async function pauseSession() {
    debugLog('⏸️ Toggling pause for BoldTake session...');
    
    try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        
        // Send pause message to content script with error handling
        if (tab?.id) {
            chrome.tabs.sendMessage(tab.id, { type: 'BOLDTAKE_PAUSE' }, (response) => {
                if (chrome.runtime.lastError) {
                    // Content script not available, but that's okay
                    debugLog('Content script not available for pause');
                }
            });
        }
        ...
        if (pauseBtn) {
            if (pauseBtn.textContent.includes('Pause')) {
                ...
                updateSessionStatus('Session paused', 'status-idle');
                addActivity('⏸️ Session paused', 'info');
            } else {
                pauseBtn.innerHTML = 'Pause Session';
                updateSessionStatus('Session resumed', 'status-running');
                addActivity('▶️ Session resumed', 'success');
            }
        }
    } catch (error) {
        debugLog('Error pausing session:', error);
    }
}
...
async function stopSession() {
    ...
    try {
        ...
        // Send stop message to content script with error handling
        if (tab?.id) {
...
async function loadSessionState() {
    try {
        ...
        // Request session stats from content script with error handling
        if (tab?.id) {
...
function updateSessionDisplay(stats) {
    ...
    // Get the current daily limit from the selected tier
    const dailyTargetInput = document.getElementById('daily-target-input');
    const dailyLimit = dailyTargetInput ? parseInt(dailyTargetInput.value) : 200;
    
    // Update progress bar with correct limit
    updateProgressBar(stats.successful || 0, dailyLimit);
...
/**
 * Updates the daily limit display based on selected tier
 */
function updateDailyLimitDisplay() {
    const dailyLimitDisplay = document.getElementById('dailyLimitDisplay');
    const dailyTargetInput = document.getElementById('daily-target-input');
    
    if (dailyLimitDisplay && dailyTargetInput) {
        const selectedLimit = dailyTargetInput.value;
        dailyLimitDisplay.textContent = `/ ${selectedLimit}`;
        
        // Also update the progress bar
        const currentCount = parseInt(document.getElementById('successfulCount').textContent) || 0;
        updateProgressBar(currentCount, parseInt(selectedLimit));
    }
}
...
function setupTabNavigation() {
    ...
    // ENSURE DASHBOARD IS SHOWN BY DEFAULT ON LOAD
    ...
    // Make sure Dashboard tab content is visible
...
async function handleCSVUpload(event) {
    ...
    try {
        ...
        const [tab] = await chrome.tabs.query({active: true, currentWindow: true});
        if ((tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) && tab?.id) {
            chrome.tabs.sendMessage(tab.id, {
                type: 'ANALYZE_REPLY_PERFORMANCE',
                data: csvData
            }, (response) => {
                if (chrome.runtime.lastError) {
                    debugLog('Content script not available for analysis');
                    return;
                }
                if (response?.success) {
    ...
    try {
        ...
        if ((tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) && tab?.id) {
            chrome.tabs.sendMessage(tab.id, {
                ...
                if (chrome.runtime.lastError) {
                    debugLog('Content script not available for insights');
                    return;
                }
                if (response?.success) {
 ...
 */
function updateProgressBar(current, goal = 200) {
    ...
    if (dailyLimitDisplay) {
        dailyLimitDisplay.textContent = `/ ${goal}`;
    }
}
...
async function loadAnalyticsData() {
    try {
        ...
        if (tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) {
            try {
                const response = tab?.id ? await chrome.tabs.sendMessage(tab.id, {type: 'GET_SESSION_STATS'}) : null;
                if (response?.stats) {
                ...
                if (tab?.url?.includes('x.com') || tab?.url?.includes('twitter.com')) {
                    ...
                    if (tab?.id) {
                        chrome.tabs.sendMessage(tab.id, {type: 'SCRAPE_ANALYTICS'}, (response) => {
                            if (chrome.runtime.lastError) {
                                debugLog('Content script not available for analytics scraping');
                                return;
                            }
                            if (response?.success) {
