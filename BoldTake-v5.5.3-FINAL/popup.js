...
function updateDailyLimitDisplay() {
    ...
    if (dailyTargetInput && dailyLimitDisplay) {
        ...
        debugLog('📊 Updated daily limit display to:', selectedValue);
    }
}
    ...
    try {
        ...
        try {
            ...
            if (tabs.length > 0) {
                ...
                debugLog('✅ Updated existing X.com tab');
            } else {
                // Create new X.com tab
                await chrome.tabs.create({ url: searchURL, active: true });
                debugLog('✅ Created new X.com tab');
            }
        } catch (error) {
            debugLog('❌ Error opening X.com tab:', error);
            // Fallback: try to create tab anyway
            try {
                await chrome.tabs.create({ url: searchURL, active: true });
                debugLog('✅ Fallback: Created new X.com tab');
            } catch (fallbackError) {
                debugLog('💥 Complete failure to open X.com tab:', fallbackError);
