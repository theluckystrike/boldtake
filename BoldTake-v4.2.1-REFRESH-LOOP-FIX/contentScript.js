...
function detectXcomErrorPage() {
  // CONSERVATIVE: Only detect ACTUAL X.com error pages, not normal content
  
  // Check for X.com's specific error page structure FIRST (most reliable)
  ...
  // If we have error page structure, that's a real error
  if (hasErrorPageStructure) {
    return true;
  }
  
  // CONSERVATIVE: Only check for very specific error combinations
  const pageText = document.body?.textContent || '';
  const pageTitle = document.title || '';
  
  // Only trigger on VERY specific X.com error page patterns
  const isActualErrorPage = (
    // X.com's specific error page title
    pageTitle.includes('Something went wrong') ||
    // X.com's specific error message with context
    (pageText.includes('Something went wrong') && pageText.includes('Try reloading')) ||
    // X.com's rate limit page
    (pageText.includes('Rate limit exceeded') && pageText.includes('Try again later')) ||
    // X.com's maintenance page
    (pageText.includes('temporarily unavailable') && pageText.includes('maintenance'))
  );
  
  // CRITICAL: Only return true for CONFIRMED error pages, not normal content
  return isActualErrorPage;
}
